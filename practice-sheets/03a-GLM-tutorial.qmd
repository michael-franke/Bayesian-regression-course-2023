...
The [Bernoulli distribution](https://michael-franke.github.io/intro-data-analysis/selected-discrete-distributions-of-random-variables.html#app-91-distributions-bernoullihael-franke.github.io/intro-data-analysis/selected-discrete-distributions-of-random-variables.html#poisson-distribution) gives the probability of a single binary outcome when the bias for the reference outcome is $p$. The link function is the logistic function (so the inverse link function is the logit function). The logistic regression model is defined as:

$$
\begin{align*}
\xi  &= X \beta       && \color{gray}{\text{[linear predictor]}} \\
\eta_i &= \text{logistic}(\xi_i) && \color{gray}{\text{[predictor of central tendency]}} \\
y_i & \sim \text{Bernoulli}(\eta_i) && \color{gray}{\text{[likelihood]}}
\end{align*}
$$
...
To run an ordinal regression model, we specify `family = cumulative(link="probit")`. The parameter `link="probit"` specifies the link function to be of the normal family (the link function is a cumulative normal distribution, the inverse of which is called "probit" function).
